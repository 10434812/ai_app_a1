...
test('fetchRealtimeQuote normalizes fund quote shape', async () => {
    await withMockedFetch((async (input) => {
        ...
        if (url.includes('fundgz.1234567.com.cn')) {
            return new Response('jsonpgz({"fundcode":"000961","name":"天弘沪深300ETF联接A","jzrq":"2026-03-20","dwjz":"1.6181","gsz":"1.5799","gszzl":"-2.36","gztime":"2026-03-23 13:18"});', { status: 200, headers: { 'Content-Type': 'application/javascript' } });
        }
...
test('fetchRealtimeQuote normalizes shop gold quote shape', async () => {
    await withMockedFetch((async (input) => {
        ...
        assert.equal(quote.extra.shopPrices[0]?.brand, '周大福');
    });
});
test('fetchRealtimeQuote normalizes bank gold quote shape', async () => {
    await withMockedFetch((async (input) => {
        ...
        assert.equal(quote.extra.bankPrices[0]?.bank, '建设银行龙鼎金条');
    });
});
