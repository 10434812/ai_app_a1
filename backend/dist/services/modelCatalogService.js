...
const buildStatusPayload = (model, input) => {
    const status = !input.isActive ? 'disabled' : input.isConfigured ? 'configured' : 'placeholder';
    const statusText = status === 'disabled' ? '已禁用' : status === 'configured' ? '已接入' : model.category === '绘图' ? '去配置图片' : '待接入';
    return {
        id: model.id,
...
export const getPublicModelStatusMap = async () => {
    const relevantKeys = Array.from(new Set([
    ...
    for (const model of ALL_MODELS) {
        ...
        if (customConfig) {
            const isConfigured = customConfig.provider === 'openai' &&
