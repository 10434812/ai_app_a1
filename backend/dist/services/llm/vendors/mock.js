...
const findLastUserQuestion = (messages) => [...messages].reverse().find((msg) => msg.role === 'user')?.content?.trim() || '';
const extractInjectionField = (block, label) => {
    const pattern = new RegExp(`^- ${escapeRegExp(label)}:\\s*(.+)$`, 'mi');
    const match = block.match(pattern);
    return match?.[1]?.trim() || '';
};
const extractRealtimeSnapshot = (block) => {
    const match = block.match(/- Ground truth snapshot:\n([\s\S]*?)\n- Instruction:/);
    return match?.[1]?.trim() || '';
};
const extractRealtimeInjection = (messages) => {
...
const buildRealtimeMockResponse = (messages) => {
