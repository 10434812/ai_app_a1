...
const resolveModelId = (modelId) => MODEL_ID_ALIASES[modelId] || modelId;
export class LLMFactory {
    static async getProvider(modelId, customApiKey) {
        const resolvedModelId = resolveModelId(modelId);
        const modelConfig = ALL_MODELS.find((m) => m.id === modelId || m.id === resolvedModelId || m.name === modelId || m.name === resolvedModelId);
        if (!modelConfig) {
            return new MockProvider();
        }
        // 0. Use Custom API Key if provided (User Override)
        if (customApiKey && modelConfig.provider === 'openai' && modelConfig.apiConfig) {
            return new OpenAICompatibleProvider(modelConfig.name, customApiKey, modelConfig.apiConfig.baseURL, modelConfig.apiConfig.modelId);
        }
