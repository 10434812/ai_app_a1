...
const parseStooqCsv = (csvLine, sourceUrl) => {
    ...
    if ([date, time].some((value) => value === 'N/D'))
        throw new Error(`Stooq quote unavailable for ${symbol}`);
...
const fetchUsdToCny = async () => {
    ...
    if (!response.ok)
        throw new Error(`USD/CNY fetch failed: ${response.status}`);
    const data = (await response.json());
    const rate = Number(data?.rates?.CNY);
    if (!Number.isFinite(rate) || rate <= 0)
        throw new Error('USD/CNY invalid');
    setCache(cacheKey, rate);
    return rate;
};
const fetchStooqSnapshot = async (symbol) => {
...
const fetchFxPair = async (pair) => {
    ...
    const quoteData = {
        ...
        sourceUrl: url,
        timestamp: Number.isFinite(Number(data?.time_last_update_unix)) && Number(data?.time_last_update_unix) > 0
...
const fetchCryptoQuote = async (id, displayName) => {
    const key = `crypto:${id}`;
    const cached = getCache(key);
    if (cached)
        return cached;
    ...
    const row = data[id];
    if (!row)
        throw new Error(`Crypto payload missing: ${id}`);
    const usd = Number(row.usd);
    const cny = Number(row.cny);
    const updatedAtUtc = Number.isFinite(Number(row.last_updated_at)) && Number(row.last_updated_at) > 0
        ? new Date(Number(row.last_updated_at) * 1000).toISOString()
        : new Date().toISOString();
    if (!Number.isFinite(usd) || usd <= 0)
        throw new Error('Crypto USD invalid');
    const quote = {
        kind: 'crypto',
        target: id,
        displayName,
...
const normalizeFundSearchKeyword = (target) => target.replace(/(今日|今天|当前|现在|最新|基金|净值|估值|多少钱|多少|行情|价格)/g, '').trim();
const resolveFundCode = async (target) => {
    if (/^\d{6}$/.test(target))
        return target;
    ...
    const response = await withTimeout(url, 4000);
    if (!response.ok)
        throw new Error(`Fund search failed: ${response.status}`);
    const data = (await response.json());
    const first = data?.Datas?.find((item) => item?.CODE && item?.NAME);
    if (!first?.CODE)
        throw new Error('Fund search empty');
    return first.CODE;
};
const parseFundEstimatePayload = (payload) => {
    const match = payload.match(/jsonpgz\((.*)\);?$/);
    if (!match?.[1])
        throw new Error('Fund estimate payload invalid');
    const parsed = JSON.parse(match[1]);
    if (!parsed.fundcode || !parsed.name)
        throw new Error('Fund estimate missing fields');
    const estimated = Number(parsed.gsz);
    if (!Number.isFinite(estimated) || estimated <= 0)
        throw new Error('Fund estimate invalid');
    return {
...
const fetchFundQuote = async (target, displayName) => {
    ...
    const response = await withTimeout(url, 4000);
    if (!response.ok)
        throw new Error(`Fund estimate fetch failed: ${response.status}`);
...
const fetchOilQuote = async (target, displayName) => {
    if (target.startsWith('CN_OIL_')) {
        ...
        const response = await withTimeout(url, 4000);
        if (!response.ok)
            throw new Error(`Domestic oil fetch failed: ${response.status}`);
        ...
        if (!hasAnyFuel)
            throw new Error('Domestic oil payload invalid');
...
const fetchShopGoldQuote = async () => {
    ...
    if (!response.ok)
        throw new Error(`Shop gold fetch failed: ${response.status}`);
    const payload = (await response.json());
    ...
    if (!shopPrices.length)
        throw new Error('Shop gold payload empty');
...
const fetchBankGoldQuote = async () => {
    ...
    if (!response.ok)
        throw new Error(`Bank gold fetch failed: ${response.status}`);
    const payload = (await response.json());
    const bankPrices = (payload?.data?.bank_gold_bar_price || [])
        ...
        .slice(0, 10);
    if (!bankPrices.length)
        throw new Error('Bank gold payload empty');
...
const fetchStockQuote = async (target, displayName) => {
    const snapshot = await fetchStooqSnapshot(target);
    const currency = target.endsWith('.HK') ? 'HKD' : target.endsWith('.CN') ? 'CNY' : 'USD';
    return {
        kind: 'stock',
        target,
        displayName,
...
export const fetchRealtimeQuote = async (intent) => {
    if (!intent.matched)
        throw new Error('Realtime intent not matched');
    switch (intent.kind) {
        case 'gold':
        ...
        default:
            throw new Error(`Unsupported realtime kind: ${String(intent)}`);
    }
};
