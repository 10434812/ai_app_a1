...
const normalizeSize = (size) => {
    ...
    if (!/^\d{3,4}x\d{3,4}$/i.test(normalized))
        return DEFAULTS.defaultSize;
    return normalized.toLowerCase();
};
const withTimeout = async (url, options, timeoutMs = 45000) => {
...
const extractImageUrls = (payload) => {
    ...
    if (Array.isArray(payload?.data)) {
        payload.data.forEach((item) => {
            pushMaybe(item?.url);
            if (item?.b64_json)
                pushMaybe(`data:image/png;base64,${item.b64_json}`);
        });
    }
    if (Array.isArray(payload?.output?.results)) {
        payload.output.results.forEach((item) => {
            pushMaybe(item?.url);
            if (item?.base64)
                pushMaybe(`data:image/png;base64,${item.base64}`);
        });
    }
...
const fetchSettings = async () => {
    ...
    const maxImagesPerRequest = parseIntSafe(map.get('IMAGE_MAX_IMAGES_PER_REQUEST') || DEFAULTS.maxImagesPerRequest, Number.parseInt(DEFAULTS.maxImagesPerRequest, 10));
...
const callOpenAICompatibleImageAPI = async (url, apiKey, payload) => {
...
const callSiliconFlowImageAPI = async (apiKey, payload) => {
    const response = await withTimeout(SILICONFLOW_IMAGE_URL, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${apiKey}`,
...
const callAliyunImageAPI = async (apiKey, payload) => {
    const size = payload.size.replace('x', '*');
    const createResponse = await withTimeout(ALIYUN_IMAGE_URL, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${apiKey}`,
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        const taskResponse = await withTimeout(`${ALIYUN_TASK_URL}/${taskId}`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${apiKey}`,
            },
        ...
        if (status === 'FAILED' || status === 'CANCELED') {
            throw new Error(`Aliyun image task ${status}: ${taskJson?.output?.message || taskJson?.message || taskJson?.code || 'unknown'}`);
        }
    }
...
export const getImageGenerationPublicOptions = async () => {
    const settings = await fetchSettings();
    const aliyunModels = Array.from(new Set([settings.aliyunModel, 'wanx2.0-t2i-turbo', 'wanx2.1-t2i-turbo', 'wan2.2-t2i-flash']));
