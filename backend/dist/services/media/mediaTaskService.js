...
export const toMediaTaskView = (task) => ({
    id: task.id,
    type: task.type,
    status: task.status,
...
const processImageTask = async (task) => {
    ...
    const actualCost = calculateImageCost(billingConfig, {
        provider: result.provider,
        model: result.model,
        modelId: parseTaskMeta(task.meta)?.modelId,
    }, promptTokens, actualCount);
...
export const createImageTask = async (input) => {
    ...
    const estimatedCost = calculateImageCost(billingConfig, {
        provider: input.provider || 'aliyun',
        model: input.model || '',
        modelId: input.modelId,
    }, estimatedPromptTokens, expectedCount);
...
export const createVideoTask = async (input) => {
    const prompt = String(input.prompt || '').trim();
    if (!prompt)
        throw new Error('Prompt is required');
    const user = await User.findByPk(input.userId);
