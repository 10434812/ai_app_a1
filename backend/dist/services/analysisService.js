...
export const getUserPortraits = async (page = 1, pageSize = 50) => {
    ...
    const portraits = users.map((user) => {
        ...
        const keywords = [];
        if (content.match(/code|program|bug|error|api|java|python|react|vue|node/i))
            keywords.push('Developer');
        if (content.match(/write|essay|story|poem|translate/i))
            keywords.push('Content Creator');
        if (content.match(/health|doctor|pain|medicine|diet/i))
            keywords.push('Health Conscious');
        if (content.match(/money|stock|invest|price|finance/i))
            keywords.push('Investor');
        if (content.match(/design|color|ui|ux/i))
            keywords.push('Designer');
        if (content.match(/marketing|seo|ad|social/i))
            keywords.push('Marketer');
        if (keywords.length === 0 && userMessages.length > 0)
            keywords.push('General User');
...
export const getCostDashboard = async () => {
    ...
    for (const record of records) {
        ...
        if (!daily[dateKey])
            daily[dateKey] = { estimatedTokens: 0, cost: 0 };
...
const ensureDailyRange = (days, start) => {
    const result = {};
...
export const getTokenEconomicsDashboard = async (daysInput = 30) => {
    ...
    for (const record of records) {
        ...
        if (usageSource === 'estimate')
            estimateUsageCount += 1;
        ...
        if (userId) {
            ...
            if (type === 'chat')
                userStats.chatCost += amount;
            if (type === 'image')
                userStats.imageCost += amount;
        ...
        if (type === 'chat')
            modelStats.chatCost += amount;
        if (type === 'image')
            modelStats.imageCost += amount;
    ...
    for (const order of orders) {
        ...
        if (order.status === 'completed') {
            ...
            if (isTokenPlan)
                tokenCompletedRevenue += amount;
            else
                membershipCompletedRevenue += amount;
            if (dailyMap[dateKey]) {
            ...
            if (isTokenPlan)
                tokenRefundedRevenue += refundAmount;
            else
                membershipRefundedRevenue += refundAmount;
            if (dailyMap[dateKey]) {
    ...
    const refundRate = completedRevenue > 0 ? Number(((Math.max(0, refundedRevenue) / completedRevenue) * 100).toFixed(2)) : 0;
