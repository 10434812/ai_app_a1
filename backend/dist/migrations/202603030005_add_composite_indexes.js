...
const createIndexIfMissing = async (sequelize, table, index, columnsSql) => {
    if (await hasIndex(sequelize, table, index))
        return;
    await sequelize.query(`CREATE INDEX \`${index}\` ON \`${table}\` (${columnsSql});`);
};
const dropIndexIfExists = async (sequelize, table, index) => {
    if (!(await hasIndex(sequelize, table, index)))
        return;
...
const migration = {
    ...
    up: async (sequelize) => {
        ...
        await createIndexIfMissing(sequelize, 'token_usage_records', 'idx_token_usage_user_type_created', '`userId`, `type`, `createdAt`');
        await createIndexIfMissing(sequelize, 'token_usage_records', 'idx_token_usage_type_model_created', '`type`, `model`, `createdAt`');
        await createIndexIfMissing(sequelize, 'conversations', 'idx_conversations_user_updated', '`userId`, `updatedAt`');
        await createIndexIfMissing(sequelize, 'messages', 'idx_messages_conversation_created_role', '`conversationId`, `createdAt`, `role`');
        await createIndexIfMissing(sequelize, 'messages', 'idx_messages_role_created_model', '`role`, `createdAt`, `model`');
        await createIndexIfMissing(sequelize, 'visit_logs', 'idx_visit_logs_visitor_path_created', '`visitorId`, `path`, `createdAt`');
    },
