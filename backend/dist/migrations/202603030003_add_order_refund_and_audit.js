...
const migration = {
    ...
    up: async (sequelize, transaction) => {
        ...
        if (!orderColumns.refundedAmount) {
            await sequelize.query(`ALTER TABLE \`${ORDER_TABLE}\` ADD COLUMN \`refundedAmount\` DECIMAL(10, 2) NULL AFTER \`status\`;`, { transaction });
        }
        if (!orderColumns.refundedAt) {
            await sequelize.query(`ALTER TABLE \`${ORDER_TABLE}\` ADD COLUMN \`refundedAt\` DATETIME NULL AFTER \`refundedAmount\`;`, { transaction });
        }
        await sequelize.query(`ALTER TABLE \`${ORDER_TABLE}\` MODIFY COLUMN \`status\` ENUM('pending','completed','failed','refunded') NOT NULL DEFAULT 'pending';`, { transaction });
        await sequelize.query(`CREATE TABLE IF NOT EXISTS \`${AUDIT_TABLE}\` (
        ...
        if (!(await hasIndex(sequelize, AUDIT_TABLE, 'idx_order_audit_logs_order_id', transaction))) {
            await sequelize.query(`CREATE INDEX \`idx_order_audit_logs_order_id\` ON \`${AUDIT_TABLE}\` (\`orderId\`);`, { transaction });
        }
        if (!(await hasIndex(sequelize, AUDIT_TABLE, 'idx_order_audit_logs_actor_user_id', transaction))) {
            await sequelize.query(`CREATE INDEX \`idx_order_audit_logs_actor_user_id\` ON \`${AUDIT_TABLE}\` (\`actorUserId\`);`, { transaction });
        }
        if (!(await hasIndex(sequelize, AUDIT_TABLE, 'idx_order_audit_logs_created_at', transaction))) {
            await sequelize.query(`CREATE INDEX \`idx_order_audit_logs_created_at\` ON \`${AUDIT_TABLE}\` (\`createdAt\`);`, { transaction });
        }
    },
    down: async (sequelize, transaction) => {
        ...
        await sequelize.query(`ALTER TABLE \`${ORDER_TABLE}\` MODIFY COLUMN \`status\` ENUM('pending','completed','failed') NOT NULL DEFAULT 'pending';`, { transaction });
    },
};
