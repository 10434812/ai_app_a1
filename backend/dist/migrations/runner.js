...
export const rollbackLastMigration = async (sequelize) => {
    await ensureMigrationsTable(sequelize);
    const [rows] = await sequelize.query(`SELECT id FROM \`${TABLE_NAME}\` ORDER BY executedAt DESC, id DESC LIMIT 1;`);
    const last = Array.isArray(rows) && rows.length > 0 ? String(rows[0].id) : '';
    if (!last) {
