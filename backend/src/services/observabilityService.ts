...
type CounterKey =
  ...
  | 'token_deduct_success_total'
  | 'token_deduct_failed_total'
  | 'image_deduct_success_total'
  | 'image_deduct_failed_total'
  | 'chat_request_total'
...
export const captureEvent = async (name: string, tags?: Record<string, string | number>) => {
...
const getCounter = async (key: CounterKey) => {
  try {
...
export const getObservabilitySummary = async () => {
  ...
  const paymentSuccessRate =
    paymentAttemptTotal > 0 ? Number(((paymentSuccessTotal / paymentAttemptTotal) * 100).toFixed(2)) : 0
  const tokenDeductFailureRate =
    tokenDeductSuccessTotal + tokenDeductFailedTotal > 0
      ? Number((tokenDeductFailedTotal / (tokenDeductSuccessTotal + tokenDeductFailedTotal) * 100).toFixed(2))
      : 0
  const imageDeductFailureRate =
    imageDeductSuccessTotal + imageDeductFailedTotal > 0
