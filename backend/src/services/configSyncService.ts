...
export interface ExportableConfigRow {
  key: string
  value: string | null
...
const isVolatileKey = (key: string) => {
  if (VOLATILE_KEY_EXACT.has(key)) return true
  return VOLATILE_KEY_PATTERNS.some((pattern) => pattern.test(key))
}

export const getExportableSystemConfigs = async (scope: 'deployment' | 'all' = 'deployment'): Promise<ExportableConfigRow[]> => {
...
export const upsertSystemConfigs = async (rows: ExportableConfigRow[]) => {
