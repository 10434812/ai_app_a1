import type {RealtimeQuote} from './realtimeQuoteService.ts'

export interface RealtimeAnswerPresentation {
  title: string
  content: string
  sourceLabel: string
...
const buildCommonFooter = (sourceLabel: string, timestampLabel: string) => [
  '',
  `数据来源：${sourceLabel}`,
  `更新时间：${timestampLabel}`,
...
export const formatRealtimeAnswer = (quote: RealtimeQuote): RealtimeAnswerPresentation => {
    ...
    case 'gold':
      if (quote.target === 'CN_SHOP_GOLD') {
        ...
        const shopPrices = Array.isArray(quote.extra.shopPrices)
          ? (quote.extra.shopPrices as Array<{brand?: string; goldPrice?: number}>).slice(0, 5)
          : []
        body = [
        ...
        const bankPrices = Array.isArray(quote.extra.bankPrices)
          ? (quote.extra.bankPrices as Array<{bank?: string; price?: number}>).slice(0, 5)
          : []
        body = [
    ...
    case 'oil':
      if (quote.target.startsWith('CN_OIL_')) {
        ...
        body = [
          ...
          typeof (quote.extra.wti as {value?: number} | undefined)?.value === 'number'
            ? `WTI：${formatNumber(Number((quote.extra.wti as {value: number}).value), 2)} USD/barrel`
            : '',
          typeof (quote.extra.brent as {value?: number} | undefined)?.value === 'number'
            ? `Brent：${formatNumber(Number((quote.extra.brent as {value: number}).value), 2)} USD/barrel`
            : '',
        ]
          .filter(Boolean)
          .join('\n')
