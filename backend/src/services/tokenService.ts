...
export const reserveUserTokenBudget = async (userId: string, requestedAmount?: number) => {
  ...
  try {
    ...
    if (reserveAmount <= 0) throw new Error('Insufficient balance')
...
export const releaseReservedTokens = async (userId: string, reservedAmount: number) => {
  if (!reservedAmount || reservedAmount <= 0) return
...
interface SettleChatReservationInput {
  userId: string
...
export const settleReservedChatUsage = async (input: SettleChatReservationInput) => {
  ...
  try {
    ...
    if (reservedAmount <= 0) throw new Error('Invalid reserved amount')
...
export const getTokenTrend = async (userId: string, days = 30) => {
   const end = new Date()
