export type RealtimeKind = 'stock' | 'fund' | 'gold' | 'oil' | 'forex' | 'crypto'

export type RealtimeIntentMatch =
  | {
      matched: true
      kind: RealtimeKind
      target: string
...
const FUND_ALIAS: Array<{re: RegExp; code: string; label: string}> = [
...
const CN_OIL_AREA_ALIAS: Array<{re: RegExp; area: string; code: string}> = [
...
const detectStockTarget = (query: string) => {
  ...
  if (/港股|hk/i.test(query)) return {symbol: `${ticker}.HK`, label: `${ticker}.HK`}
  if (/a股|上证|深证|沪深|cn/i.test(query)) return {symbol: `${ticker}.CN`, label: `${ticker}.CN`}
  return {symbol: `${ticker}.US`, label: `${ticker}.US`}
}

const detectFundTarget = (query: string) => {
  ...
  if (!FUND_QUERY_RE.test(query)) return null
...
const detectFxPair = (query: string) => {
  ...
  if (FOREX_QUERY_RE.test(query)) return {base: 'USD', quote: 'CNY'}
  return null
}

const detectCryptoTarget = (query: string) => {
  const alias = CRYPTO_ALIAS.find((item) => item.re.test(query))
  if (alias) return alias
  if (CRYPTO_QUERY_RE.test(query)) return {id: 'bitcoin', label: '比特币'}
  return null
}

export const detectRealtimeIntent = (query: string): RealtimeIntentMatch => {
