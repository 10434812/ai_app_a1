...
const readConfigMap = async (keys: string[]) => {
...
export interface WeChatPayConfigTestResult {
  success: boolean
  mode: 'mock' | 'live'
  message: string
  missing: string[]
...
export interface WeChatNativeCheckoutResult {
  type: 'native'
  code_url: string
  mock?: boolean
}
...
class WeChatPayService {
  private async resolveConfig(override?: Partial<WeChatPayConfig>): Promise<WeChatPayConfig> {
  ...
  async getConfig(override?: Partial<WeChatPayConfig>): Promise<WeChatPayConfig> {
    return this.resolveConfig(override)
  }

  async getConfigState(override?: Partial<WeChatPayConfig>) {
    ...
    if (!config.certPem) missing.push('WECHAT_PAY_CERT_PEM')
  ...
  private async createClient(override?: Partial<WeChatPayConfig>) {
    ...
    if (config.mockMode) {
      return {config, client: null as WxPay | null}
    }
  ...
  async testConfiguration(override?: Partial<WeChatPayConfig>): Promise<WeChatPayConfigTestResult> {
  ...
  async createNativeTransaction(orderId: string, description: string, amount: number): Promise<WeChatNativeCheckoutResult> {
  ...
  async createJsapiTransaction(
    ...
    amount: number,
    openid: string,
  ): Promise<WeChatJsapiCheckoutResult> {
    const {config, client} = await this.createClient()
