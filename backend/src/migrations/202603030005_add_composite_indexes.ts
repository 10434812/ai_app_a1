...
const createIndexIfMissing = async (sequelize: Sequelize, table: string, index: string, columnsSql: string, transaction?: Transaction) => {
  if (await hasIndex(sequelize, table, index, transaction)) return
  await sequelize.query(`CREATE INDEX \`${index}\` ON \`${table}\` (${columnsSql});`, {transaction})
}

const dropIndexIfExists = async (sequelize: Sequelize, table: string, index: string, transaction?: Transaction) => {
  if (!(await hasIndex(sequelize, table, index, transaction))) return
