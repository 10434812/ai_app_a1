...
import {ADMIN_ROLES, authenticateToken, requireAdmin} from '../middleware/auth.ts'
...
import {ApiError, sendError} from '../errors/api.ts'
...
const resolveOrderIdFromWeChatOutTradeNo = (value: string) => {
  ...
  if (!trimmed) return ''
  if (!WECHAT_OUT_TRADE_NO_REGEX.test(trimmed)) return trimmed
...
paymentRouter.get('/usage', authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const daily: Record<string, {messages: number; estimatedTokens: number}> = {}
    for (let i = 0; i < 7; i++) {
    ...
    for (const record of records) {
      ...
      if (!daily[key]) daily[key] = {messages: 0, estimatedTokens: 0}
...
paymentRouter.get('/orders', authenticateToken, async (req: Request, res: Response) => {
  const userId = req.user!.id
  try {
...
paymentRouter.post('/checkout', authenticateToken, withRateLimit('payment'), async (req: Request, res: Response) => {
  const userId = req.user!.id
  const {planKey, method = 'wechat'} = req.body

  if (!planKey || !PAYMENT_PLANS[planKey as PlanKey]) {
    sendError(res, {
      status: 400,
  ...
  const selectedPlanKey = planKey as PlanKey
  const plan = PAYMENT_PLANS[selectedPlanKey]
  const requestToken =
    readIdempotencyToken(req) ||
  ...
  try {
    ...
    const idemResult = await runIdempotent(
      idemKey,
      async () => {
        ...
        if (method === 'wechat_jsapi') {
          ...
          try {
            const result = await weChatPayService.createJsapiTransaction(
              buildWeChatOutTradeNo(order.id),
              `AI App - ${plan.name}`,
              plan.amount,
              user.wechatOpenId,
            )
...
// Check Order Status
paymentRouter.get('/status/:orderId', authenticateToken, async (req: Request, res: Response) => {
...
paymentRouter.post('/mock-success', requireAdmin, async (req: Request, res: Response) => {
...
paymentRouter.post('/wechat/notify', async (req: Request, res: Response) => {
  try {
    const rawBody =
      Buffer.isBuffer(req.body) ? req.body.toString('utf-8') : typeof req.body === 'string' ? req.body : JSON.stringify(req.body || {})
...
export async function processPaymentSuccess(orderId: string, options?: {transactionId?: string}) {
  try {
    await withOrderLock(orderId, 'payment_success', async () => {
      await sequelize.transaction(async (transaction) => {
        ...
        const planFromConfig = planKey ? PAYMENT_PLANS[planKey as PlanKey] : null
        ...
        if (!order.planKey) order.planKey = planKey
        if (!order.planSnapshot && planFromConfig) {
          order.planSnapshot = makePlanSnapshot(planKey as PlanKey, planFromConfig)
        }
