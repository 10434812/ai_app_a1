...
import {ALL_MODELS} from '../services/llm/config.ts'
import {SystemConfig} from '../models/SystemConfig.ts'
import {countActiveModels, getModelStatusMap, setModelActive} from '../services/modelStatusService.ts'
...
import {PlanKey, PAYMENT_PLANS, getPlanName, inferPlanKeyFromOrderPlan, makePlanSnapshot, parsePlanSnapshot} from '../config/paymentPlans.ts'
import {adjustUserTokens, recordTokenUsage} from '../services/tokenService.ts'
import {calculateChatCost, getBillingConfig, updateBillingConfig} from '../services/billingConfigService.ts'
import {getObservabilitySummary} from '../services/observabilityService.ts'
...
import {MEMBERSHIP_PLAN_MATRIX} from '../config/membership.ts'
import {processPaymentSuccess} from './payment.ts'
...
const requireActionConfirmation = (req: Request, res: Response, actionLabel: string) => {
...
const buildOrderSnapshot = (order: Order) => ({
  id: order.id,
  userId: order.userId,
  amount: order.amount,
  plan: order.plan,
...
router.get('/orders/audits', requirePermission('audit:read'), async (req: Request, res: Response) => {
  try {
...
router.post('/orders/:id/repair', requirePermission('orders:operate'), async (req: Request, res: Response) => {
  try {
    if (!requireActionConfirmation(req, res, '修复订单状态')) return
    const orderId = req.params.id
    const status = String(req.body?.status || '').trim() as Order['status']
...
router.post('/orders/:id/manual-complete', requirePermission('orders:operate'), async (req: Request, res: Response) => {
  try {
    if (!requireActionConfirmation(req, res, '手动补单')) return
    const orderId = req.params.id
    const note = String(req.body?.note || '').trim()
    const actorUserId = req.user?.id || 'unknown'
...
router.post('/orders/:id/manual-refund', requirePermission('orders:operate'), async (req: Request, res: Response) => {
  try {
    if (!requireActionConfirmation(req, res, '手动退款')) return
    const orderId = req.params.id
    const note = String(req.body?.note || '').trim()
      ...
      async () => {
        return withOrderLock(orderId, 'manual_refund', async () => {
          return sequelize.transaction(async (transaction) => {
            ...
            const plan = (planKey ? PAYMENT_PLANS[planKey as PlanKey] : null) || snapshot
...
// Config Keys
router.get('/config/keys', requirePermission('settings:manage'), async (req: Request, res: Response) => {
  try {
...
router.post('/config/keys', requirePermission('settings:manage'), async (req: Request, res: Response) => {
...
// General Settings
router.get('/config/general', requirePermission('settings:read'), async (req: Request, res: Response) => {
  try {
    const keys = [
      'site_name',
      'enable_registration',
...
router.post('/config/general/wechat-pay/test', requirePermission('settings:manage'), async (req: Request, res: Response) => {
  try {
...
router.post('/config/general', requirePermission('settings:manage'), async (req: Request, res: Response) => {
  try {
...
router.get('/config/export', requirePermission('settings:read'), async (req: Request, res: Response) => {
  try {
...
router.post('/config/import', requirePermission('settings:manage'), async (req: Request, res: Response) => {
  try {
...
// Billing Config
router.get('/billing/config', requirePermission('billing:read'), async (_req: Request, res: Response) => {
  try {
...
router.post('/billing/config', requirePermission('billing:manage'), async (req: Request, res: Response) => {
  try {
...
router.post('/billing/preview', requirePermission('billing:read'), async (req: Request, res: Response) => {
  try {
...
// Metrics
router.get('/metrics', requirePermission('dashboard:view'), async (req: Request, res: Response) => {
  const metrics = await getMetrics()
  const activeModelsCount = await countActiveModels()
  res.json({...metrics, activeModels: activeModelsCount})
})

router.get('/observability', requirePermission('dashboard:view'), async (_req: Request, res: Response) => {
  try {
...
// Analysis Routes
router.get('/analysis/stats', requirePermission('analysis:read'), async (req: Request, res: Response) => {
  try {
...
router.get('/analysis/questions', requirePermission('analysis:read'), async (req: Request, res: Response) => {
  try {
...
router.get('/analysis/costs', requirePermission('analysis:read'), async (req: Request, res: Response) => {
  try {
...
router.get('/analysis/retention', requirePermission('analysis:read'), async (req: Request, res: Response) => {
  try {
...
router.get('/analysis/economics', requirePermission('analysis:read'), async (req: Request, res: Response) => {
  try {
...
router.get('/export/tokens.csv', requirePermission('export:read'), async (req: Request, res: Response) => {
  try {
...
router.post('/data/archive', requirePermission('archive:execute'), async (req: Request, res: Response) => {
  try {
...
router.patch('/users/:id/toggle', requirePermission('users:manage'), async (req: Request, res: Response) => {
  const {id} = req.params
...
router.delete('/users/:id', requirePermission('users:manage'), async (req: Request, res: Response) => {
  try {
    if (!requireActionConfirmation(req, res, '删除用户')) return
...
router.delete('/conversations/:id', requirePermission('users:manage'), async (req: Request, res: Response) => {
  try {
    if (!requireActionConfirmation(req, res, '删除对话日志')) return
...
router.post('/conversations/batch-delete', requirePermission('users:manage'), async (req: Request, res: Response) => {
  try {
    if (!requireActionConfirmation(req, res, '批量删除对话日志')) return
...
// Models
router.get('/models', requirePermission('models:read'), async (req: Request, res: Response) => {
  try {
...
router.post('/models/:id/config', requirePermission('models:manage'), async (req: Request, res: Response) => {
  const {id} = req.params
...
router.patch('/models/:id/toggle', requirePermission('models:manage'), async (req: Request, res: Response) => {
  const {id} = req.params
