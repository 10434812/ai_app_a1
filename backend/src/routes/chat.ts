...
import {releaseReservedTokens, reserveUserTokenBudget, settleReservedChatUsage} from '../services/tokenService.ts'
import redisClient from '../config/redis.ts'
import {Conversation} from '../models/Conversation.ts'
import {Message} from '../models/Message.ts'
import {User} from '../models/User.ts'
import {authenticateToken, optionalAuthenticateToken} from '../middleware/auth.ts'
import {withConcurrencyGuard, withEntitlement, enforceModelSelectionLimit} from '../middleware/entitlement.ts'
import {buildRealtimeContextMessage, buildRuntimeGuardrailMessage} from '../services/realtimeContextService.ts'
import {calculateChatCost, estimateTextTokens, getBillingConfig} from '../services/billingConfigService.ts'
import {captureError, metricCounters} from '../services/observabilityService.ts'
...
const resolveModelAlias = (modelId: string) => MODEL_ID_ALIASES[modelId] || modelId

const mergeSystemMessagesToFront = (messages: Array<{role?: string; content?: string}>) => {
  const systemContents: string[] = []
  const normalMessages: Array<{role: 'user' | 'assistant'; content: string}> = []
  ...
  if (systemContents.length === 0) return normalMessages
...
const getSelectedModelCount = (value: unknown) => {
  const parsed = Number.parseInt(String(value || ''), 10)
  if (!Number.isFinite(parsed) || parsed <= 0) return 1
  return parsed
}

const calculateReservationTarget = (balance: number, minCost: number, selectedModelCount: number) => {
...
const getRoutedModelName = (modelId: string) => {
...
const buildRouteMeta = async (modelId: string) => {
  const resolvedId = resolveModelAlias(modelId)
  const modelDef = ALL_MODELS.find((m) => m.id === modelId || m.id === resolvedId)
...
const buildModelIdentityGuardrailMessage = (modelId: string) => {
  const resolvedId = resolveModelAlias(modelId)
...
const isModelIdentityQuery = (query: string) => {
...
const buildModelIdentityReply = async (modelId: string) => {
  const resolvedId = resolveModelAlias(modelId)
  const modelDef = ALL_MODELS.find((m) => m.id === modelId || m.id === resolvedId)
...
const isImageLikeAssistantMessage = (msg: Message): boolean => {
  if (msg.role !== 'assistant') return false

  const modelId = typeof msg.model === 'string' ? msg.model : ''
  if (IMAGE_ASSISTANT_MODEL_IDS.has(modelId)) return true
...
chatRouter.get('/history', authenticateToken, async (req: Request, res: Response) => {
...
chatRouter.patch('/:conversationId/favorite', authenticateToken, async (req: Request, res: Response) => {
...
chatRouter.patch('/batch/archive', authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    const normalizedIds = Array.from(
      new Set(conversationIds.map((id) => String(id).trim()).filter(Boolean)),
    )
    ...
    await Conversation.update(
      {isArchived, ...(isArchived ? {isFavorite: false} : {})},
      {
        where: {
...
chatRouter.patch('/:conversationId/archive', authenticateToken, async (req: Request, res: Response) => {
  const {conversationId} = req.params
...
chatRouter.get('/:conversationId/messages', authenticateToken, async (req: Request, res: Response) => {
  const {conversationId} = req.params
  const userId = req.user!.id
...
chatRouter.put('/:conversationId', authenticateToken, async (req: Request, res: Response) => {
  const {conversationId} = req.params
...
chatRouter.delete('/:conversationId', authenticateToken, async (req: Request, res: Response) => {
  const {conversationId} = req.params
  const userId = req.user!.id
...
chatRouter.post('/batch-delete', authenticateToken, async (req: Request, res: Response) => {
...
chatRouter.post('/message', optionalAuthenticateToken, async (req: Request, res: Response) => {
  ...
  const role: 'user' | 'assistant' = requestedRole === 'assistant' ? 'assistant' : 'user'
...
chatRouter.post('/', optionalAuthenticateToken, withRateLimit('chat'), withEntitlement, enforceModelSelectionLimit, withConcurrencyGuard('ai'), async (req: Request, res: Response) => {
  const {
    model,
    messages,
  ...
  const releaseReservationSafely = async () => {
    if (!user || tokenSettled || reservedAmount <= 0) return
    try {
    ...
    try {
      ...
      const filteredHistory = history.filter((msg) => {
        ...
        if (msg.role === 'assistant') {
          if (isImageLikeAssistantMessage(msg)) return false
  ...
  const latestUserQuery = [...messages].reverse().find((m: any) => m?.role === 'user')?.content || ''
  ...
  inputLenForCost = contextMessages.reduce((acc: number, m: any) => acc + (m?.content?.length || 0), 0)
  const estimatedPromptTokens = estimateTextTokens(contextMessages.map((m: any) => m?.content || '').join('\n'))
  ...
  try {
    ...
    const streamResult = await provider.chatStream(contextMessages, (chunk) => {
      chunkCount++
      if (chunkCount === 1) console.log(`[FirstChunk] Model: ${model} Time: ${Date.now()}`)
    ...
    if (user) {
      ...
      let usageSource: 'provider' | 'estimate' = 'provider'
