...
const DEFAULTS: Record<RateScope, RateLimitOptions> = {
  chat: {
    scope: 'chat',
...
const parseList = (value: string | undefined | null) =>
  new Set(
    String(value || '')
      .split(/[,\n]/)
      .map((item) => item.trim())
...
const parsePeakHours = (raw?: string | null): [number, number] => {
  ...
  if (!Number.isFinite(start) || !Number.isFinite(end)) return [9, 23]
  return [Math.min(23, Math.max(0, start)), Math.min(23, Math.max(0, end))]
}

const safeFactor = (value: unknown, fallback: number, min = 0.2, max = 4) => {
  const parsed = Number(value)
  if (!Number.isFinite(parsed)) return fallback
  return Math.min(max, Math.max(min, parsed))
}

let dynamicConfigCache: {expiresAt: number; value: DynamicRateLimitConfig} | null = null

const getDynamicConfig = async (): Promise<DynamicRateLimitConfig> => {
  ...
  const next: DynamicRateLimitConfig = {
    ...
    reputationThreshold: Math.max(
      1,
      Number.parseInt(
        map.get('RATE_LIMIT_REPUTATION_THRESHOLD') || process.env.RATE_LIMIT_REPUTATION_THRESHOLD || '20',
        10,
      ) || 20,
    ),
    reputationPenalty: safeFactor(
      map.get('RATE_LIMIT_REPUTATION_PENALTY') || process.env.RATE_LIMIT_REPUTATION_PENALTY,
      0.7,
    ),
    ...
    premiumFactor: safeFactor(
      map.get('RATE_LIMIT_FACTOR_PREMIUM') || process.env.RATE_LIMIT_FACTOR_PREMIUM,
      2.4,
    ),
  }
...
const increaseWithWindow = async (key: string, ttlSeconds: number) => {
...
const getMembershipLevel = async (req: Request): Promise<'free' | 'pro' | 'premium'> => {
  const levelFromEntitlement = req.entitlement?.membershipLevel
  if (levelFromEntitlement === 'pro' || levelFromEntitlement === 'premium') return levelFromEntitlement
...
const getMembershipFactor = (level: 'free' | 'pro' | 'premium', config: DynamicRateLimitConfig) => {
  if (level === 'premium') return config.premiumFactor
  if (level === 'pro') return config.proFactor
  return config.freeFactor
}

const getTimeFactor = (config: DynamicRateLimitConfig) => {
...
export const withRateLimit = (scope: RateScope, custom?: Partial<RateLimitOptions>) => async (req: Request, res: Response, next: NextFunction) => {
