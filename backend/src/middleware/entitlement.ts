...
const attachEntitlement = async (req: Request) => {
...
export const withEntitlement = async (req: Request, _res: Response, next: NextFunction) => {
  try {
...
const getRequestedModelCount = (req: Request) => {
  const models = req.body?.models
  if (Array.isArray(models)) return models.length
  if (req.body?.model) return 1
  return 0
}

export const enforceModelSelectionLimit = (req: Request, res: Response, next: NextFunction) => {
  if (!req.entitlement) return next()
...
export const enforceImageRequestLimit = (req: Request, res: Response, next: NextFunction) => {
  if (!req.entitlement) return next()
...
const withConcurrencyKey = (channel: GuardChannel, userId: string) => `concurrency:${channel}:${userId}`

export const withConcurrencyGuard = (channel: GuardChannel = 'ai') => async (req: Request, res: Response, next: NextFunction) => {
  if (!req.entitlement) return next()
